...
flowchart TD
    subgraph FASE1["Fase 1: Ingesta y Resguardo"]
        ...
        L_Ingesta -->|"2. Guarda XML"| S3_Main[("S3 Bucket Principal<br>(Región Actual)")]
    end
    ...
    subgraph FASE2["Fase 2: Consultas Batch y Fallback (Event-Driven)"]
        ...
        EB -->|"4. Encola Consultas"| SQS["Amazon SQS<br>(Queue + DLQ)"]
        
        SQS -->|"5. Triggers Batch Execution"| SF["AWS Step Functions<br>(State Machine)"]
    ...
    subgraph REGION_HISTORICA["Región Secundaria (Histórico)"]
        S3_Hist[("S3 Bucket Histórico<br>(En Migración por otro equipo)")]
    end
